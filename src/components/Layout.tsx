import type { ReactNode } from "react";
import { DoodleWhats } from "@/components/Doodles";
import { Logo } from "@/components/Logo";
import {
  MAILTO,
  PAISES,
  WHATSAPP_DISPLAY,
  WHATSAPP_URL,
  nombrePais,
  whatsappPais,
  type Idioma,
} from "@/site";

const HEADER_TXT = {
  es: {
    inicio: "Papurro, inicio",
    paises: "Países",
    wa: (n: string) => `Escribir por WhatsApp al ${n}`,
    ctaLarga: "diagnóstico gratis →",
    ctaCorta: "diagnóstico →",
    portal: "portal de cliente",
  },
  en: {
    inicio: "Papurro, home",
    paises: "Countries",
    wa: (n: string) => `Message on WhatsApp: ${n}`,
    ctaLarga: "free diagnostic →",
    ctaCorta: "diagnostic →",
    portal: "client portal",
  },
} as const;

const CLIENT_PORTAL_URL = "https://client.papurro.com";

/** Barra superior: marca, países (desktop), WhatsApp directo y el CTA al diagnóstico. */
export function Header({
  activeSlug,
  ctaHref = "/#diagnostico",
  pais,
  idioma = "es",
}: {
  activeSlug?: string | undefined;
  /** Ancla al diagnóstico: relativa si la página lo tiene, absoluta si no. */
  ctaHref?: string | undefined;
  /** País de la landing, para que el WhatsApp llegue diciendo de dónde salió. */
  pais?: string | undefined;
  idioma?: Idioma | undefined;
}) {
  const wa = pais ? whatsappPais(pais, idioma) : WHATSAPP_URL;
  const t = HEADER_TXT[idioma];

  return (
    <header className="sticky top-0 z-40 border-b border-hairline bg-background/85 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
        <a href="/" className="flex items-center gap-2" aria-label={t.inicio}>
          <Logo className="h-11 w-auto" placeholderClassName="text-xl" />
        </a>

        <nav aria-label={t.paises} className="hidden items-center gap-1 md:flex">
          {PAISES.map((p) => (
            <a
              key={p.slug}
              href={`/${p.slug}/`}
              aria-current={activeSlug === p.slug ? "page" : undefined}
              className={`rounded-full px-3 py-2 font-display text-sm lowercase transition-colors hover:bg-[color-mix(in_oklab,var(--ink)_6%,white)] ${
                activeSlug === p.slug ? "bg-[color-mix(in_oklab,var(--ink)_7%,white)]" : ""
              }`}
            >
              {nombrePais(p, idioma).toLowerCase()}
            </a>
          ))}
        </nav>

        <div className="flex items-center gap-2">
          <a
            href={CLIENT_PORTAL_URL}
            className="btn-press hidden rounded-full bg-paper px-4 py-2.5 font-display text-sm shadow-soft ring-1 ring-hairline sm:inline-flex sm:items-center"
          >
            {t.portal}
          </a>

          <a
            href={wa}
            target="_blank"
            rel="noopener noreferrer"
            aria-label={t.wa(WHATSAPP_DISPLAY)}
            title={`WhatsApp ${WHATSAPP_DISPLAY}`}
            className="btn-press bg-paper p-2.5 shadow-soft ring-1 ring-hairline"
          >
            <DoodleWhats className="h-6 w-6 text-ink" />
          </a>

          <a href={ctaHref} className="btn-press btn-rainbow px-5 py-2.5 text-base">
            <span className="hidden sm:inline">{t.ctaLarga}</span>
            <span className="sm:hidden">{t.ctaCorta}</span>
          </a>
        </div>
      </div>
    </header>
  );
}

const FOOTER_TXT = {
  es: {
    inicio: "Papurro, inicio",
    resumen:
      "Automatización, IA aplicada, atención al cliente y optimización para tiendas de " +
      "e-commerce de Uruguay, Argentina y Estados Unidos. Una persona, no una agencia.",
    paisesAria: "Países atendidos",
    dondeTrabajo: "dónde trabajo",
    ecommerceEn: (n: string) => `E-commerce en ${n}`,
    contactoAria: "Contacto y legales",
    contacto: "contacto",
    whatsapp: (n: string) => `WhatsApp ${n}`,
    privacidad: "Privacidad",
    respondo: "Respondo en 24-48 h hábiles",
    hecho: (year: number) => `Hecho en Uruguay 🧉 · © ${year} Papurro`,
  },
  en: {
    inicio: "Papurro, home",
    resumen:
      "Automation, applied AI, customer support and optimization for e-commerce stores " +
      "in Uruguay, Argentina and the United States. One person, not an agency.",
    paisesAria: "Countries served",
    dondeTrabajo: "where I work",
    ecommerceEn: (n: string) => `E-commerce in ${n}`,
    contactoAria: "Contact and legal",
    contacto: "contact",
    whatsapp: (n: string) => `WhatsApp ${n}`,
    privacidad: "Privacy",
    respondo: "I reply within 24-48 business hours",
    hecho: (year: number) => `Made in Uruguay 🧉 · © ${year} Papurro`,
  },
} as const;

export function Footer({ idioma = "es" }: { idioma?: Idioma | undefined }) {
  const t = FOOTER_TXT[idioma];

  return (
    <footer className="mt-6 bg-ink px-4 py-10 text-paper sm:px-6">
      <div className="mx-auto grid max-w-6xl gap-8 sm:grid-cols-[1.2fr_1fr_1fr]">
        <div>
          <a href="/" aria-label={t.inicio}>
            <Logo className="h-9 w-auto shrink-0" placeholderClassName="text-2xl" />
          </a>
          <p className="mt-3 max-w-xs text-xs leading-relaxed text-paper/70 sm:text-sm">
            {t.resumen}
          </p>
        </div>

        <nav aria-label={t.paisesAria}>
          <h2 className="font-display text-sm text-paper">{t.dondeTrabajo}</h2>
          <ul className="mt-3 space-y-2 text-xs text-paper/70 sm:text-sm">
            {PAISES.map((p) => (
              <li key={p.slug}>
                <a href={`/${p.slug}/`} className="underline underline-offset-4 hover:text-paper">
                  {t.ecommerceEn(nombrePais(p, idioma))} {p.bandera}
                </a>
              </li>
            ))}
          </ul>
        </nav>

        <nav aria-label={t.contactoAria}>
          <h2 className="font-display text-sm text-paper">{t.contacto}</h2>
          <ul className="mt-3 space-y-2 text-xs text-paper/70 sm:text-sm">
            <li>
              <a href={MAILTO} className="underline underline-offset-4 hover:text-paper">
                info@papurro.com
              </a>
            </li>
            <li>
              <a
                href={WHATSAPP_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="underline underline-offset-4 hover:text-paper"
              >
                {t.whatsapp(WHATSAPP_DISPLAY)}
              </a>
            </li>
            <li>
              <a href="/privacidad/" className="underline underline-offset-4 hover:text-paper">
                {t.privacidad}
              </a>
            </li>
            <li className="text-paper/60">{t.respondo}</li>
          </ul>
        </nav>
      </div>

      <p className="mx-auto mt-8 max-w-6xl text-xs text-paper/60">
        {t.hecho(new Date().getFullYear())}
      </p>
    </footer>
  );
}

export function Page({
  children,
  activeSlug,
  ctaHref,
  pais,
  idioma = "es",
}: {
  children: ReactNode;
  activeSlug?: string | undefined;
  ctaHref?: string | undefined;
  pais?: string | undefined;
  idioma?: Idioma | undefined;
}) {
  return (
    <div className="min-h-screen bg-background text-ink">
      <Header activeSlug={activeSlug} ctaHref={ctaHref} pais={pais} idioma={idioma} />
      <main id="top" className="mx-auto max-w-6xl space-y-6 px-4 py-6 sm:px-6 sm:py-10">
        {children}
      </main>
      <Footer idioma={idioma} />
    </div>
  );
}
