...
- Real contact channels, and only these: the email **info@papurro.com** and WhatsApp at **+598 92 061 005**. No fake phone numbers, no fake address.
...
**1. Sticky top bar** — mascot + "papurro" left; right side: a WhatsApp icon button and the main CTA "diagnóstico gratis →", anchored to `#diagnostico`.
...
**5b. Diagnóstico de 20 minutos** (`#diagnostico`, the section every CTA points at)
The entry offer: a free, no-strings 20-minute call. Three cards (short call / free and no commitment /
you leave with something either way), then the contact form and the WhatsApp shortcut. Defined once in
`DIAGNOSTICO` in `src/site.ts`, which also feeds the JSON-LD `Offer` (price 0) and `llms.txt`.
It is a real offer — keep the copy honest and do not attach conditions that are not actually applied.
...
- React + Tailwind + TypeScript. **No backend, no database, no auth, no third-party form service.**
- The contact form (`src/components/Formulario.tsx`) has no server: it collects the fields in React,
  builds a pre-written `mailto:` with `mailtoConsulta()` and hands it to the visitor's mail client.
  Nothing is stored or transmitted by the site. Fields are length-capped so the resulting URL stays
  under the ~2000-char limit mail clients truncate at.
  Because a visitor with no mail client configured sees nothing happen, the form always shows the
  direct channels (email + WhatsApp) as a fallback after submit — do not remove that.
- To swap in a real form endpoint later (Formspree, Web3Forms, …), replace the `enviar` handler; the
  markup and the `Consulta` type stay as they are.
